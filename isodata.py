...
def isodata(df, exp_k, least_n, max_its, max_pair, theta_s, theta_c):
    list_c = []              # 存储所有的cluster
    sigma_k = 0.5            # 分裂常数
    n_c = 5                  # 聚类中心个数
    ...
    while True:
        ...
        if n_c <= args.exp_k/2 or (n_c < 2*args.exp_k and iter_num % 2 == 1):
            ...
            # step 10 分裂操作
            for center_j in range(n_c):
                if max_sigmas[center_j] > args.theta_s:
                    if (list_c[center_j].inner_avg_distance > total_avg_distance and list_c[center_j].sample_num > 2*(args.least_n+1)) or n_c <= args.exp_k/2:
                        new_center_1 = list_c[center_j].center.copy()
                        new_center_1[max_indexes[center_j]] += sigma_k * max_sigmas[center_j]
                        new_center_2 = list_c[center_j].center.copy()
                        new_center_2[max_indexes[center_j]] -= sigma_k * max_sigmas[center_j]
...
cluster_list = isodata(df_, exp_k_, least_n_, max_its_, max_pair_, theta_s_, theta_c_)
